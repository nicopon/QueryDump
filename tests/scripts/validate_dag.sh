...
run_sql_test "T4: SQL JOIN" \
  "-i \"parquet:$A/t4_main.parquet\" --alias main -i \"csv:$A/t4_ref.csv\" --column-types \"Id:double,Label:string\" --alias ref --from main --ref ref --sql \"SELECT main.*, ref.Label FROM main LEFT JOIN ref ON main.Id = ref.Id\"" \
  check_t4
...
[ "$COUNT_A" -ge 1 ] && pass "Join+fan-out (duckdb): output A OK" || fail "Join+fan-out (duckdb): output A empty"
[ "$COUNT_B" -ge 1 ] && pass "Join+fan-out (duckdb): output B OK" || fail "Join+fan-out (duckdb): output B empty"
...
echo "  [SQL Engine: DuckDB] T11: UUID → Parquet..."
...
"$DTPIPE" -i "parquet:$A/t11_uuid.parquet" --alias p --from p --sql "SELECT CAST(CAST(Id AS UUID) AS VARCHAR) AS Id FROM p" -o "$A/t11_uuid_verify.csv" --no-stats \
  || fail "UUID Parquet (duckdb): Parquet read-back failed"

count=$(csv_rows "$A/t11_uuid_verify.csv")
[ "$count" -eq 10 ] && pass "UUID Parquet (duckdb): 10 rows round-tripped" || fail "UUID Parquet (duckdb): got $count rows"
...
grep -qiE "[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}" "$A/t11_uuid_verify.csv" \
  && pass "UUID Parquet (duckdb): values are valid UUID strings" \
  || fail "UUID Parquet (duckdb): UUID values corrupted or missing"
...
check_t12_all() {
    ...
    "$DTPIPE" -i "generate:1000" --alias m -i "generate:10" --alias r --from m --ref r \
        --sql "SELECT m.* FROM m JOIN r ON m.GenerateIndex = r.GenerateIndex" \
        -o "$A/t12_out.csv" --no-stats
    check_t12 "$A/t12_out.csv" "$eng"
}

echo "  [SQL Engine: DuckDB] T12: Nested JOINs..."
check_t12_all "dummy" "duckdb"
...
echo "  [SQL Engine: DuckDB] T14: UUID value fidelity..."
...
"$DTPIPE" -i "parquet:$A/t14_out.parquet" -o "$A/t14_verify.csv" --no-stats \
  || fail "UUID fidelity (duckdb): Parquet read-back failed"
